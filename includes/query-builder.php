...
class BP_Docs_Query {
		...
		if ( $doc_id = (int)$this->query_args['doc_id'] ) {
			...
			if ( !empty( $this->query_args['search_terms'] ) ) {
				...
				if ( bp_docs_enable_attachments() ) {
					...
					if ( $attachment_posts ) {
						$this->attachment_parent_ids = wp_list_pluck( $attachment_posts, 'post_parent' );
	...
	public function attachment_filename_search_filter( $search ) {
		...
		if ( ! $search || empty( $this->attachment_parent_ids ) ) {
			return $search;
		}

		$raw_search = preg_replace( '/^\s*AND/', '', $search );
		$parent_ids = implode( ',', array_map( 'intval', $this->attachment_parent_ids ) );
